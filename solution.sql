...
--Insert Data from a CSV Files into the SQL Tables
copy courses from '‪D:/sql evening/project/courses.csv' delimiter ',' csv header;

copy enrollments from 'D:/sql evening/project/enrollments.csv' delimiter ',' csv header;

copy instructors from 'D:/sql evening/project/instructors.csv' delimiter ',' csv header;

copy students from 'D:/sql evening/project/students.csv' delimiter ',' csv header;

--(Part 1: Basic Queries)
--1.Retrieve the title and price of all courses in the 'Data Science' category that cost less than 2000. 
--Order the results from the cheapest to the most expensive.
...
--3.Identify the categories where the total number of enrollments is greater than 150. 
--Display the category name and the total enrollment count.
...
--7.Display the name of every instructor. If an instructor is teaching any courses, also list the title of their courses. 
--If an instructor is not teaching any courses, their name should still appear with a blank or NULL course title.
...
--15.For each course category, assign a rank to each course based on its price (highest price gets rank 1). 
--The ranking should reset for each category. Display the course title, category, price, and the calculated rank.
select title, category, price,
rank() over (partition by category order by price desc) as price_rank from courses;

--16.Calculate the difference between each course's price and the average price of all courses within its own category. 
--Display the course title, price, category average price, and the calculated difference.
...
--17.Write a PostgreSQL function called get_course_enrollment_count that accepts a course_id as an integer input and returns 
--the total number of enrollments for that specific course as an integer output.
...
--19.Create a trigger that automatically logs the course_id, old_price,and new_price into the course_price_audit table 
--whenever the price of a course is updated in the courses table.
...
--20.Create a stored procedure (or a function with LANGUAGE plpgsql) called add_new_enrollment that inserts a new record into
--the enrollments table using three parameters: p_student_id (INT), p_course_id (INT), and an optional p_enrollment_date (DATE). 
--If no date is provided, the procedure must use the current date.
...
--22.Find the name of all students whose name contains 'Jackson' (case-insensitive). 
--Display their name in uppercase and their email address.
select upper(name) as name, email from students where name ILIKE '%jackson%';

--23.Create a new PostgreSQL user role named auditor. Grant this role SELECT access on the enrollments table. 
--Revoke the SELECT privilege on the city column of the students table from the auditor role.
create role auditor;
...
--24.Create a view named v_instructor_performance that shows the instructor name, the title of the courses they teach, 
--and the total number of enrollments for each of those courses.
...
--25.Create a non-unique index on the category column of the courses table. 
--Briefly explain the performance benefit of creating this index.
